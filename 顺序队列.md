...
<img width="404" height="322" alt="image" src="https://github.com/user-attachments/assets/efdab23c-276e-487b-9057-3a0ca0f428cd" />

****************************************************** [矩阵中字符‘1’构成的最大正方形的面积](https://leetcode.cn/problems/maximal-square/description/) ***************************************************/  
* 数形结合 左右两边寻找连接部分
*     int row = matrix.length , col = matrix[0].length;
      int[] heigths = new int[col];
      for(int i = 0 ; i < row ; i++){
        for(int j = 0 ; j < col ; j++){
          if(matrix[i][j] == '0') heights[j] = 0;
          else heigths[j] += 1;
        }
        ans = Math.max(ans , caculate(heigths);
      }

      int caculate(int[] heigths){
        int n = heigths.length , res = 0;
        int[] leftMin = new int[n] , rightMin = new int[n];
        Queue<Integer> leftQue = new ArrayDeque() , rightQue = new ArrayDeque();
        for(int i = 0 ; i < n ; i++){
          int left = i , right = n - 1 - i;
          while(!leftQue.isEmpty() && heigths[leftQue.peek()] > heigths[right]){
            leftMin[leftQue.pop()] = right;
          )
          leftQue.push(right);
          while(!rightQue.isEmpty() && heigths[rightQue.peek()] < heigths[left]){
            rightMin[rightQue.pop()] = left;
          )
          rightQue.push(left);
        }
        while(!leftQue.isEmpty()){
          leftMin[leftQue.peek()] = -1;
        }
        while(!rightQue.isEmpty()){
          rightMin[rightQue.pop()] = n;
        }
        for(int i = 0 ; i < n ; i++){
          int heigth = heigths[i];
          int button = rightMin[i] - leftMin[i] + 1;
          int l = Math.min(heigth , button);
