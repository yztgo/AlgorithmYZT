         ...
         boolean[] primes = new boolean[max];
            for(int i = 2 ; i < max ; i++){
                ...
                for(int j = 0 ; j < ans.size() ; j++){
                    ...
                    if(num >= max) break;
                    primes[num] = true;
                    if(i % ans.get(j) == 0) break; // 保证每一个合数都是被其最大因数筛选出来的 比如说12 在i == 4时候会提前终止 保证12是被i == 6进行筛选出来的
                }
            }
        return ans.size();
...
******************************** 检验某一个数是不是丑数 ****************************************
* 由于除1外的所有数都是由2 3 5的多个幂次乘积得到的 所以直接检验即可 而判断一个数是不是2的整数幂次又存在特定的解法(数的二进制表示 与 位运算）
*                while( n % 3 == 0){
                     n / = 3;
                 }
                 while( n % 5 == 0){
...
******************************** 判断某一个数是否可以表示成一个质数K的幂次 **************************************
* 找到小于$`2^{31} - 1`$的最大的k幂次 ， 在该范围内的所有成立的数都可以整除该数
*        long mx = 30 , max = Integer.MAX_VALUE:
         while(Math.pow(k , mx--) > max){};
         mx = Math.pow(k , mx);
         return mx % num == 0;


******************************** 判断某一个数是否可以表示成3的不同幂次和 **************************************
* n % 3 = [0 (成立） , 1 == $`3^0`$（成立） , 2（不成立）]
*        while(n > 0){
...
********************* 完全平方数（背包问题） 判断某一个数是否可以表示为一些数的平方和的形式 **********************

********************* 位运算的一些小技巧 计算机底层的二进制存储 三码合一 ***************************************
* 计算机底层只认识 0 和 1，所有数值最终都以补码形式存储（这是 “三码合一” 的核心 —— 正数的原码 = 反码 = 补码，负数则通过补码（反码加一）统一运算规则）。
* [构造最小位运算数组](https://leetcode.cn/problems/construct-the-minimum-bitwise-array-ii/description/)
