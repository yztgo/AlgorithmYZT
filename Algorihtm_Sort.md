...
*     Java 的排序实现（以 Arrays.sort() 为例）会根据 数据类型 和 数据规模 选择不同的算法：
      ...
      使用 TimSort（归并 + 插入排序混合）
      2. 为什么小数据用插入排序？
      (1) 算法复杂度 vs 常数开销
      归并排序：需要额外的临时数组 ，并且递归拆分、合并的过程有较高的常数开销。
      插入排序：实现简单、内存访问局部性好，常数开销极低。
      对于 几十个元素以内 的数据，插入排序往往比归并排序更快。
      (2) CPU 缓存友好
      插入排序是 原地排序，访问模式连续，能充分利用 CPU 缓存（Cache locality），减少内存访问延迟。
      
      (3) 分支预测更准确
      插入排序的比较和移动模式在小规模数据中更规律，CPU 分支预测命中率高，执行效率好。
      
      3. TimSort / 双轴快排中的插入排序优化
      Java 的排序实现并不是“纯插入排序”，而是：
      
      大规模数据 → 先用快排或归并排序分成小块
      小块数据（<47）→ 用插入排序完成排序
      这种混合策略结合了 插入排序在小规模数据的高效性 和 快排/归并在大规模数据的优势。
