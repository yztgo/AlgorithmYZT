 ...
 *   右侧累加树 [右侧展开搜索二叉树](https://leetcode.cn/problems/NYBBNL/description/) 中序遍历的特点 实现题目要求

/************************************** 二叉树的路径总和 - 树上前缀和 ， 树节点之间的关系 ， 字符串二叉树 **************************************/
* 后序遍历 左右节点遍历之后进行值得选取 或 目标对象的收集 等等判断 类似于动态规划中的max min 加法的判断 也就是哪个动态规划的转移方程项
* [二叉树中的最大路径和](https://leetcode.cn/problems/jC7MId/description/) 
*         int ans = Integger.MIN_VALUE ;
  ...
  思考 二叉树中等于target的路径的个数

* [二叉树中等于target的路径数（路径必须从上到下）](https://leetcode.cn/problems/path-sum-iii/description/)
*          Map<Long , Integer> cnt = new HashMap();
           int ans = 0;
           cnt.put(0L , 1);
           dfs(root , 0L , cnt , target);

           void dfs(TreeNode root , long last , Map<Long , Integer> cnt , int target){
               if(root == null) return ;
               long cur = (long)last + root.val;
               ans += cnt.getOrDefault(cur - target , 0);
               cnt.merge(cur , 1 , Long::sum);
               dfs(root.left , cur , cnt , target);
               dfs(root.right , cur , cnt , target);
               cnt.merge(cur , -1 , Long::sum);

<img width="571" height="302" alt="image" src="https://github.com/user-attachments/assets/e59a6075-a484-443c-9f6d-5adb8c89870e" />

* [最长同值路径](https://leetcode.cn/problems/longest-univalue-path/description/)
*      public int longestUnivaluePath(TreeNode root) {
        ...
        int dfs(TreeNode root , int last){
            if(root == null) return 0;
            int res = 1 , cur = root.val , left = dfs(root.left , cur) , right = dfs(root.right , cur);
...
*************************************** 二叉树中的返回对象 **************************************/
* 递归中的返回对象是递归的第一要义 [包含所有最深节点的最小子树](https://leetcode.cn/problems/smallest-subtree-with-all-the-deepest-nodes/description/?envType=daily-question&envId=2026-01-09)
...
# 原理：将小集合依附于不小于它的集合
/*************************************** 判断图是否是一个无环图 *****************************/
* 二维数组edges : int[n][2] 表示存在n条无向边 由于n个节点组成的无环图只存在n-1条边 所以请你剔除其中的一条边使得剩余的n-1条边能够组成树，而不仅仅是无环图
...
* 
<img width="848" height="585" alt="image" src="https://github.com/user-attachments/assets/1850bd82-901b-4361-936c-4ed18672c7d0" />
...
*      boolean[] cnt ;
       ...
       return true;
/************************************** 以邻接表的形式表示二叉树 ***************************/       
** [删除某一个节点后 每个剩余子树中所有节点个数和 再乘积 的最大值的个数](https://leetcode.cn/problems/count-nodes-with-the-highest-score/description/)


/************************************** 无向图中的单向dfs ***************************/
* 给你一个表示n个城市之间连接关系的邻接矩阵，请你找出这些城市可以被分为几类。
* 虽然可以使用并查集（上面的规则1 edge[i] -> edge[j] （j > i) 注意最后使用循环find(i)遍历 使得每一个连通块中的节点都指向根节点
  因为每一次都是节点所在连通块的根节点进行相连，按照规则1中顺序连接可能会出现深度是2的树。
...
/*************************************** Kruskal最小生成树  ***************************/  
* 找到带权无向图中的最小生成树，删除某些边不破坏图的连通性并且剩余的边权重和最小
*         Class Union{
              private int[] dp;
              private int num;
              private int[][] mintree ;
              private int totalLen ;
              public void minTree(int[][] edges){
                  ...
                  int n = cnt.size();
                  this.num = n;
                  this.ans = new int[n][3];
                  for(int i = 0 ; i < n ; i++){
                      this.dp[i] = i;
                  }
                  Arrays.sort(edges , (a , b) -> a[2] - b[2]); // 按照权重的降序进行排序
                  for(int[] edge : edge){
